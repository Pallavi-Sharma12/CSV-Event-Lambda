    ...
    try:
        ...
        if keyName.endswith('.csv'):
            try:
                ...
                csvContent = csvfile['Body'].read()
                df = pd.read_csv(io.BytesIO(csvContent), low_memory=False)
            ...
            toDB_Values = [(row['batch'], row['start'], row['end'], row['records'], row['pass'], row['message']) for index, row in df.iterrows()]
            cur.executemany("INSERT INTO csvfile (batch, start, end, records, pass, message) VALUES (?, ?, ?, ?, ?, ?);", toDB_Values)
            con.commit()
            con.close()
        else:
